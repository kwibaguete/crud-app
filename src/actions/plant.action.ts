...
export async function getPlants(searchTerm?: String) {
    try {
        ...
        //make plants visible/callable to corresponding users who made them
        const whereClause: any = {
            userId: currentUserId,
        };

        //make search terms not case sensitive
        if (searchTerm) {
            whereClause.name = {
                contains: searchTerm,
                mode: "insensitive",
        ...
        //cache this plant so that when you refresh the page you don't have to get it again, 
        // aka re cache the page to udpate it for the user
        revalidatePath("/");
...
export async function createPlant(data: Prisma.PlantsCreateInput) {
  ...
  try {
    ...
    //cache new data so make page faster
